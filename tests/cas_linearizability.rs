...
fn cas_client(
    ...
    Task::local(async move {
        ...
        let mut witnessed: Vec<VersionedValue> =
            vec![last_known.clone()];
...
fn cas_linearizability() {
    ...
    let clients =
        vec![
            cas_client
                as fn(Client) -> Task<Vec<VersionedValue>>;
            n_clients
        ];
...
fn basic() {
    ...
    fn basic_client_ops(mut client: Client) -> Task<()> {
        Task::local(async move {
            ...
            println!(
                "majority pinger got {} responses",
                responses
            );

            let set = client
                .set(b"k1".to_vec(), b"v1".to_vec())
                .await;
    ...
    let clients =
        vec![basic_client_ops as fn(Client) -> Task<()>];
